...
%Generate Initial Velocity Guess from assumptions that frictionFactor = 0.45
...
%     frictionFactor = 1/(2*lambertw( log(10)/(2*a)*10^(b/(2*a)))/log(10) - (b/a))^2;
    ...
    if (reCheck > 3999)
        a = 2.51/Re;
        frictionFactor = 1/(2*lambertw( log(10)/(2*a)*10^(b/(2*a)))/log(10) - (b/a))^2;
%         frictionFactor = 1/(2*lambertw( log(10)/(2*(2.51/(Re)))*10^(roughness/14.8*(diameter/2)/(2*(2.51/(Re)))))/log(10) - (roughness/14.8*(diameter/2)/(2.51/(velocity*rho*diameter/viscosity))))^2;
    else
